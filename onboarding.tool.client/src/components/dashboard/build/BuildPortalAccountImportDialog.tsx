...
import { buildPortalImport } from "../../../pages/dashboard/build/types/buildPortalImport";
...
interface BuildPortalAccountImportDialogProps {
  open: boolean;
  onCancel: () => void;
  onImport: () => void;
}
...
function parseCSV(text: string): {
  ...
  const data = lines.slice(1).map((line, idx) => {
    ...
    return row as buildPortalImport;
  });

  return { data, errors };
}

export const BuildPortalAccountImportDialog: FC<
  BuildPortalAccountImportDialogProps
> = ({ open, onCancel, onImport }) => {
  const { loading, selectedInstance } = useInstanceContext();
  const { setPortalAccounts } = useBuildPageContext();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [csvErrors, setCsvErrors] = useState<string[]>([]);
  const [csvData, setCsvData] = useState<buildPortalImport[] | null>(null);
  ...
  const handleUpload = async () => {
    ...
    const endpoint: string = `/api/v1/dashboard/instance/${selectedInstance.host}/build/portal/import`;
    const result: buildPortalAccount[] = await api.post(endpoint, csvData, {
      credentials: "include",
    });

    if (result.length == 0) {
      showSnackbar("Failed to import any portal accounts", "error");
      return;
    } else if (result.length != csvData.length) {
      showSnackbar("One or more accounts have failed to import", "warning");
    } else {
  ...
  return (
    <Dialog open={open} onClose={onCancel} maxWidth="sm" fullWidth>
      <DialogTitle>Import Portal Accounts</DialogTitle>
      <DialogContent>
        <Box mb={2}>
          ...
          <Link
            href={`${import.meta.env.VITE_BASE_URL || "/"}sample-portal.csv`}
            download
          >
            Download sample CSV
          </Link>
        ...
        {csvData && csvErrors.length === 0 && (
          <Alert severity="success" sx={{ mt: 2 }}>
            CSV looks good! Ready to upload {csvData.length} portals.
          </Alert>
        )}
      </DialogContent>
      <DialogActions>
