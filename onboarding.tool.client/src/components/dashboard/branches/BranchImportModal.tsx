...
interface BranchImportDialogProps {
  open: boolean;
  onCancel: () => void;
  onImport: (branches: importBranch[]) => void;
}
...
function parseCSV(text: string): {
  ...
  const data = lines.slice(1).map((line, idx) => {
    ...
    if (row.salesEmail && !/^\S+@\S+\.\S+$/.test(row.salesEmail)) {
      errors.push(`Row ${idx + 2}: Invalid email in 'salesEmail'`);
    }

    if (row.lettingsEmail && !/^\S+@\S+\.\S+$/.test(row.lettingsEmail)) {
      errors.push(`Row ${idx + 2}: Invalid email in 'lettingsEmail'`);
    }

    if (row.salesPhone && !/^\+44\d{10}$/.test(row.salesPhone)) {
      errors.push(`Row ${idx + 2}: Invalid phone number in 'salesPhone'`);
    }

    if (row.lettingsPhone && !/^\+44\d{10}$/.test(row.lettingsPhone)) {
      errors.push(`Row ${idx + 2}: Invalid phone number in 'lettingsPhone'`);
    }

    return row as importBranch;
  });

  return { data, errors };
}

export const BranchImportDialog: FC<BranchImportDialogProps> = ({
  open,
  onImport,
  onCancel,
}) => {
  ...
  const [csvData, setCsvData] = useState<importBranch[] | null>(null);
  ...
  const handleUpload = async () => {
    ...
    const endpoint: string = `/api/v1/dashboard/instance/${selectedInstance.host}/branch/import`;
    const result: branch[] = await api.post(endpoint, csvData, {
      credentials: "include",
    });

    if (result.length == 0) {
      showSnackbar("Failed to import any branches", "error");
      return;
    } else if (result.length != csvData.length) {
      showSnackbar("One or more branches have failed to import", "warning");
    } else {
    ...
    <Dialog open={open} onClose={onCancel} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Box mb={2}>
          ...
          <Link
            href={`${import.meta.env.VITE_BASE_URL || "/"}sample-branches.csv`}
            download
          >
            Download sample CSV
          </Link>
        ...
        {csvData && csvErrors.length === 0 && (
          <Alert severity="success" sx={{ mt: 2 }}>
            CSV looks good! Ready to upload {csvData.length} branches.
          </Alert>
        )}
      </DialogContent>
      <DialogActions>
